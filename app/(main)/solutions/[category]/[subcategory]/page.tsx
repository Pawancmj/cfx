        ...
        <main className="min-h-screen relative overflow-hidden text-zinc-100 section-bg-dark">
            ...
            <section className="relative min-h-screen flex items-center border-b border-white/5 overflow-hidden section-bg-dark pt-24">
                <div className="container mx-auto px-6 lg:px-8 relative z-10">
                    ...
                    >
                        <Link
                            href={`/solutions/${categoryData.slug}`}
                                    ...
                                    <div className="relative rounded-3xl overflow-hidden glass-card border border-white/10 shadow-2xl flex items-center justify-center p-2 bg-zinc-900/50 aspect-video lg:aspect-[4/3]">
                                        <div className="w-full h-full relative rounded-2xl overflow-hidden bg-zinc-950">
                                            <Image 
                                                ...
                                                alt={detailData.heroTitle || detailData.title}
                                                fill
            ...
            {detailData.challengesVsSolutions && detailData.challengesVsSolutions.length > 0 && (
                <section className="py-24 relative z-10 border-b border-white/5 bg-zinc-950/20">
                        ...
                        <div className="grid gap-8 lg:grid-cols-2">
                            {detailData.challengesVsSolutions.map((item: any, idx: number) => (
                                <MotionDiv
                                    ...
                                    className="flex flex-col md:flex-row gap-0 rounded-[2rem] overflow-hidden glass-card border border-white/5 group shadow-2xl relative"
                                >
                                    {/* Background glow on hover */}
                                    <div className="absolute inset-0 bg-gradient-to-r from-red-500/5 to-primary/5 opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none"></div>

                                    {/* Challenge Side (Red/Warning tint) */}
                                    <div className="flex-1 p-8 bg-black/40 border-b md:border-b-0 md:border-r border-white/5 relative overflow-hidden group-hover:bg-black/60 transition-colors">
                                        <div className="absolute top-0 left-0 w-1 h-full bg-red-500/50"></div>
                                        <div className="flex items-center gap-3 mb-4">
                                            <div className="w-8 h-8 rounded-full bg-red-500/10 flex items-center justify-center border border-red-500/20 text-red-500 shadow-[0_0_10px_rgba(239,68,68,0.2)]">
                                                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                                            </div>
                                            <h4 className="text-xs font-bold text-red-400 uppercase tracking-[0.2em]">The Challenge</h4>
                                        </div>
                                        <p className="text-zinc-400 text-sm font-medium leading-relaxed group-hover:text-zinc-300 transition-colors">{item.challenge}</p>
                                    </div>

                                    {/* Solution Side (Primary/Green tint) */}
                                    <div className="flex-1 p-8 bg-primary/5 relative overflow-hidden group-hover:bg-primary/10 transition-colors duration-500">
                                        <div className="absolute top-0 left-0 md:left-auto md:right-0 w-full h-1 md:w-1 md:h-full bg-primary/50"></div>
                                        
                                        <div className="flex items-center gap-3 mb-4 relative z-10">
                                            <div className="w-8 h-8 rounded-full bg-primary/20 flex items-center justify-center border border-primary/30 text-primary shadow-[0_0_15px_rgba(0,242,255,0.3)]">
                                                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>
                                            </div>
                                            <h4 className="text-xs font-bold text-primary uppercase tracking-[0.2em]">Our Solution</h4>
                                        </div>
                                        <p className="text-white text-sm font-semibold leading-relaxed relative z-10">{item.solution}</p>
                                    </div>
                                </MotionDiv>
                            ))}
                        </div>
                            ...
                            {relatedSolutions.map((solution: any, idx: number) => (
                                <Link href={`/solutions/${categoryData.slug}/${solution.slug}`} key={idx} className="block group h-full">
                                    <MotionDiv
                                        ...
                                        viewport={{ once: true }}
                                        transition={{ duration: 0.5, delay: idx * 0.1 }}
                                        className="h-full glass-card p-8 rounded-3xl border border-white/5 group-hover:border-primary/30 group-hover:bg-white/5 transition-all duration-300 relative overflow-hidden flex flex-col"
                                    >
                                        <div className="absolute inset-0 bg-gradient-to-br from-primary/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
                                        ...
                                        <div className="mt-auto flex items-center text-xs font-bold text-primary uppercase tracking-wider relative z-10">
                                            Explore Solution <ArrowRight className="w-4 h-4 ml-2 group-hover:translate-x-1 transition-transform" />
                                        </div>
                                    </MotionDiv>
                                </Link>
                            ))}
            ...
            {/* Final CTA */}
            <section className="py-16 md:py-20 relative z-10 border-t border-white/5">
                <div className="container mx-auto px-6 lg:px-8 text-center">
                    <MotionDiv
                        ...
                        viewport={{ once: true }}
                        transition={{ duration: 0.8 }}
                        className="p-10 md:p-16 rounded-[3.5rem] section-bg-gradient text-center relative overflow-hidden group shadow-2xl border border-white/10"
                    >
